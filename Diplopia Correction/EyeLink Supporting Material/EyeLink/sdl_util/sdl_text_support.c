...
{
	int i =0; 
	for(; fontmap[i].filename && fontmap[i].font_face && fontmap[i].font_name; i++)
	{
...
#elif defined(MACOSX) && defined(MACAPP)
	static char wd[1024] = {0};
	if(!strlen(wd)) 
	{
		CFURLRef appRef = CFBundleCopyBundleURL(CFBundleGetMainBundle());
		CFStringRef appPath = CFURLCopyFileSystemPath(appRef,kCFURLPOSIXPathStyle);
		const char *pathPtr = CFStringGetCStringPtr(appPath,CFStringGetSystemEncoding());
		strcpy(wd,pathPtr);
...
int get_new_font(char *fontname, int font_size, int bold)
{
	  ...
	  if(!font)
	  {
		  printf("could not load from %s %s\n", fontpath, SDL_GetError());
		  return -1;
	  }
	  if(bold)
...
void sdl_printf(SDL_Surface *surface, char * text, int format, 
					int x, int y, SDL_Color foreground)
{
  ...
  if(!font)
  {
	...
	if(x+width > surface->w-marginw)
	{
		...
		if(!stop) 
		{// no line breaks found. break at a white space
			int i =0; 
			for(i =MAX_CHAR_PERLINE-2; i >=0 ; i--)
			{
				...
				if( (temp[i] == ' ' || temp[i] == '\t') &&  w+x < surface->w-marginw )
				{
					stop2 =i;
					break;
				}
		...
		if(stop)
		{
			...
			if(format & WRAP && strlen(temp) && strlen(temp) < strlen(text))
			{
				...
				sdl_printf(surface,text+i,format, x,y+(int)((double)height*(linespacing)),foreground);
			}
    ...
    if(strlen(text) > MAX_CHAR_PERLINE)
      text[MAX_CHAR_PERLINE-1] = 0; // chop the line.
		strcpy(temp,text);
		textsurface = TTF_RenderText_Solid(font,temp,foreground);
		if(!textsurface)
		{
			printf("Could not create text surface \n");
			return ;
 ...
 * Output :
 *******************************************************************************/
void graphic_printf(SDL_Surface *surface, SDL_Color fg, int format, int x, int y, const char *fmt, ...)
{
	va_list argptr;
