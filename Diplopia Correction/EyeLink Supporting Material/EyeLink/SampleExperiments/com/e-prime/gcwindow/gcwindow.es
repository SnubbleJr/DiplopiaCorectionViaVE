...
[UserScript]
'
' Copyright (c) 1997 - 2006 by SR Research Ltd., All Rights Reserved        
'                                                                           
...
Sub doGCWindow(fgName as String, bgName as String, DURATION as Integer, trialid as String, trial_word as String)
	...
	waitBlock = tracker.waitForBlockStart(1000,True,False)  'wait for link sample data
	...
	if eye = 2 then    'if both eye's data present: use left eye only
		eye = 0
	end if
	...
	while elutil.currentTime() < stime + DURATION and button = 0 and tracker.isConnected() and not tracker.breakPressed()
		...
		Set s = tracker.getNewestSample()    'check for new sample update

		' make sure we get a new sample
...
cancelout:
	...
	'Each trial can have a set of such interest areas. 
	'The following example draws a 100 * 100 pixels interest area in the center of the screen
	tracker.sendMessage "!V IAREA RECTANGLE 1 " & Display.XRes/2 -50 & " " & Display.YRes/2 - 50 & " " & Display.XRes/2 + 50 & " " & Display.YRes/2 + 50 & " " & TrialList.GetCurrentAttrib("fgImageName")
